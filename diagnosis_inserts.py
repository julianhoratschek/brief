...
def get_diagnosis_inserts(patient: Patient):
    ...
    # Ready-made run properties
    rpr18 = DocxRunProperties([font, DocxSizeProperty(18)])
    rpr16 = DocxRunProperties([font, DocxSizeProperty(16)])

    # Ready-made paragraph properties
    ppr_diagnoses = [DocxIndentationProperty(1134, 1134),
                     jc,
                     rpr18]
    ppr18 = [rpr18]
    ppr18_jc = [jc, rpr18]
    ...
    for icd10, name in patient.diagnosis:
        ...
        # Find paragraphs associated with this diagnosis
        match icd10:
            ...
            case 'G43.1':
                ...
                migraine_with_aura_letter_recommendations = [
                    ...
                    .run(" ist der Einsatz von Triptanen während einer Aura kontraindiziert. Hier empfiehlt sich "
                         "die Einnahme von Akutanalgetika wie Novaminsulfon® (Metamizol) 40° bis zu 4x täglich. "
            ...
            case 'G44.0':
                ...
                cluster_base_recommendations = [
                    ...
                    .run("Kontinuierlich Kopfschmerzkalender führen"),

                    DocxParagraph(ppr_num)
                    .run("Die Dosierung des Verapamils sollte dem Krankheitsverlauf angepasst werden. Bei "
                         "unzureichender Wirkung kann ggf. eine weitere Dosissteigerung unter Beachtung von "
                         "Verträglichkeit und Nebenwirkungsspektrum erfolgen, die Tagesdosis von 960 mg pro "
                         "Tag nicht überschreitend"),

                    DocxParagraph(ppr_num)
                    .run("Unter der aktuellen Verapamil-Medikation bitten wir um regelmäßige kardiologische Kontrollen "
                         "mittels EKG und Echokardiographie sowie bei jeder Dosissteigerung"),

                    DocxParagraph(ppr_num)
                    .run("Wegen des chronischen Verlaufs der Clusterkopfschmerzen ist die Einnahme von Verapamil "
                         "zeitlich nicht befristet. Dosisreduktion des Verapamils nach 4-6 attackenfreien Wochen, "
                         "hierbei schrittweise ausdosieren. Bei erneutem Ausbrechen von Clusterattacken erneute "
                         "schrittweise Aufdosierung"),

                    DocxParagraph(ppr_num)
                    .run("Wir bitten um kardiologische Kontrolluntersuchung der o.g. Medikation mit Echokardiographie, "
                         "Langzeitbelastungs-EKG zeitnah durchzuführen"),

                    DocxParagraph(ppr_num)
                    .run("Da Clusterkopfschmerz eine Schmerzform ist die überdurchschnittlich häufig bei Rauchern "
                         "auftritt und durch Alkohol triggerbar ist, empfahlen wir dringend eine Nikotin- und "
                         "Alkoholabstinenz"),
                    ...
                    DocxParagraph(ppr_num)
                    .run("Dosiserhöhung des Lithiums an die Attackenhäufigkeit angepasst. Maximaler Blutspiegel "
                         "0,6 mmol/l. Eine Dosisreduktion des Lithiums frühestens nach 4-6 attackenfreien Wochen. "
                         "Hierbei schrittweise ausdosieren. Dosisreduktion von Verapamil nach Absetzen des Lithiums "
                         "von weiteren 4-6 attackenfreien Wochen, hierbei nur schrittweise ausdosieren. Bei erneutem "
                         "Ausbruch von Clusterattacken erneute schrittweise Aufdosierung", rpr16.having(yellow)),

                    DocxParagraph(ppr_num)
                    .run("Die Bestimmung des Serumlithiumspiegels sollte in den ersten 4 Wochen wöchentlich "
                         "vorgenommen werden, danach ggf. bei weiterer Einnahme im ersten halben Jahr 1x monatlich "
                         "und später im vierteljährlichen Abstand. Die Bestimmung des Serumlithiumspiegels sollte "
                         "möglichst genau 12 Stunden nach der letzten Einnahme erfolgen. Zwecksmäßigerweise wird die "
                         "Bestimmung am Morgen vor der weiteren Tablettengabe durchgeführt. Wir bitten um eine "
                         "sorgfältige Überwachung des Patienten während der Lithiummedikation. Folgende "
                         "Untersuchungen werden gemäß Fachinformation jährlich empfohlen: T3, T4, TSH basal, ggf. "
                         "TAH-Test, Natrium, Kalium und Calciumbestimmung, 24 Stundenurinvolumen, Kreatinin-Clearens, "
                         "EKG, EEG, Urinanalyse, Blutdruckmessung, Blutbild und ggf. Überprüfung der renalen "
                         "Konzentrationsleistung, Desmopressin-Test und eine vierteljährliche Messung von "
                         "Körpergewicht und Halsumfang. Bitte kardiologische Kontrolluntersuchung mit "
                         "Echokardiographie, Langzeit- und Belastungs-EKG zeitnah durchführen lassen",
                         rpr16.having(yellow)),
                ]
                ...
                cluster_letter_recommendations = [
                    ...
                    .run("Die Dosierung des Verapamil retard (Isoptin®) sollte dem Krankheitsverlauf angepasst werden. "
                         "Bei unzureichender Wirkung kann ggf. eine weitere Dosissteigerung unter Beachtung vom "
                         "Verträglichkeits- und Nebenwirkungsspektrum erfolgen, die Tagesdosis von 960 mg pro Tag "
                         "nicht überschreitend. Unter Verapamil-Medikation bitten wir um regelmäßige kardiologische "
                         "Kontrollen mittels EKG und Echokardiographie sowie bei jeder Dosissteigerung. "
                         "Dosisreduktion des Verapamils nach 6-8 attackenfreien Wochen, hierbei "
                         "schrittweise Ausdosieren."),

                    empty_paragraph,
                    DocxParagraph(ppr18_jc)
                    .run("Die Attackenbehandlung des Clusterkopfschmerzes kann durch Inhalation von Sauerstoff "
                         "15 l/min über 15 Minuten unter Verwendung einer Gesichtsmaske sowie bei Wirkungslosigkeit "
                         "durch die Verwendung eines Triptans mit schnellem Wirkungseintritt, z.B. Migra-Pen® "
                         "(Sumatriptan 3 mg) als Autoinjektor s.c. oder 4 mg Fertigspritze erfolgen. Hier ist darauf "
                         "zu achten, dass die Maximaldosis von Sumatriptan in 24 Stunden 12 mg s.c. beträgt. Im "
                         "Gegensatz zur Migränebehandlung mit Triptanen besteht jedoch keine Höchstgrenze der "
                         "Anwendung in Tagen pro Monat, da medikamenteninduzierte Kopfschmerzen bei "
                         "Triptanübergebrauch auf dem Boden von Clusterkopfschmerzen bisher nicht beschrieben sind.")
                ]
            ...
            case 'G44.2':
                ...
                tth_letter_recommendations = [
                    ...
                    .run(" sind Verhaltensmaßnahmen in Form von Stressreduktion, Entspannungsverfahren, Sporttherapie, "
                         "Biofeedback, Wärmeanwendungen, Massageanwendungen sowie ggf. eine Behandlung einer "
                         "oromandibulären Dysfunktion ein zentraler Baustein."),

                    empty_paragraph,

                    DocxParagraph(ppr18_jc)
                    .run("Die hier vermittelten nicht-medikamentöse Therapieoptionen bei ")
                    .run("Kopfschmerzen vom Spannungstyp", rpr18.having(big))
                    .run(" sollten auch ambulant fortgesetzt werden. Diese beinhalten eine Reduktion psychischer "
                         "Stressoren, eine Reduktion muskulärer Stressoren, die Behandlung von Angst und Depression "
                         "sowie die Therapie einer oromandibulären Dysfunktion. Die diesbezüglichen Strategien "
                         "schließen Entspannungsverfahren wie die Progressive Muskelrelaxation, im Biofeedback "
                         "erlernte Strategien, Stressbewältigungskompetenzen, Lerneinheiten aus Patientenseminaren "
                         "sowie sporttherapeutische Aktivitäten ein. Physikalische Therapiemaßnahmen umfassen die "
                         "Thermotherapie, Physiotherapie, TENS-Behandlung sowie Reiztherapie. Üblicherweise ist der "
                         "chronische Kopfschmerz vom Spannungstyp nur nach mehrmonatiger intensiver und nachhaltiger "
                    ...
                    DocxParagraph(ppr18_jc)
                    .run("Kopfschmerzen vom Spannungstyp", rpr18.having(big))
                    .run(" sollten möglichst nur in Ausnahmefällen, maximal jedoch an 10 Tagen im Monat analgetisch "
                         "behandelt werden, um die Entstehung eines medikamenteninduzierten Dauerkopfschmerzes zu "
                         "vermeiden. Die medikamentöse Akuttherapie muss darauf ausgerichtet sein, einen Kopfschmerz "
                         "bei Medikamentenübergebrauch als Komplikation zu vermeiden. Daher ist vorzugsweise die "
                         "Anwendung von Pfefferminzöl in alkoholischer Lösung (z. B. Euminz N) zu empfehlen, "
                         "Non-Opioid-Analgetika und Opioid-Analgetika im eigentlichen Sinn sollten vermieden werden. "
                         "Zur Therapiekontrolle sollte der Kopfschmerzkalender oder die Migräne-App kontinuierlich "
                         "geführt werden, um sowohl Kopfschmerzsymptome, Medikamenteneinnahme als auch Therapieeffekte "
                         "im Verlauf zu protokollieren."),
                ]
            ...
            case 'G44.4':
                ...
                overuse_base_recommendations = [
                    ...
                    .run(". Medikamentenpause heißt: Alle Medikamente für die Akutbehandlung von Kopfschmerzen dürfen "
                         ...
                         "Akutmedikation behandelt werden. Eine medikamentöse Attackentherapie kann auch dann wieder "
                         "beginnen. Nach Abschluss der Analgetikapause sollte eine Reevaluation der Kopfschmerzen und "
                         "entsprechende Diagnosesicherung erfolgen.")
                ]
                ...
                overuse_letter_recommendations = [
                    DocxParagraph(ppr_hi).run("Mit Cortison:", rpr18_hi_big),

                    DocxParagraph(ppr_hi)
                    .run("Wir führten eine konsequente ")
                    .run("Medikamenten-Einnahmepause", rpr18_hi_big)
                    .run(" für jegliche Kopfschmerzakutmedikation durch. Zur Erleichterung der zu erwartenden "
                         "Umstellungsreaktion erfolgte eine befristete intravenöse und orale Gabe von Prednisolon. "
                         "Zum Einsatz kamen ebenfalls Dimenhydrinat und Melperon, sowie Infusionen mit Dimenhydrinat "
                         "und Magnesium."),

                    empty_paragraph,

                    DocxParagraph(ppr_hi).run("Ohne Cortison:", rpr18_hi_big),

                    DocxParagraph(ppr_hi)
                    .run("Wir führten eine konsequente ")
                    .run("Medikamenten-Einnahmepause", rpr18_hi_big)
                    .run(" für jegliche Kopfschmerzakutmedikation durch. Dabei wurde auf eine intravenöse und orale "
                         "Gabe von Prednisolon nach einem festen Zeitschema verzichtet. Bedarfsweise erhielt die "
                         "Patientin Medikamente zur Schmerzdistanzierung.")
                ]

                overuse_closing_statement = [
                    DocxParagraph(ppr_hi)
                    .run("Des Weiteren empfehlen wir die ambulante Fortführung der ")
